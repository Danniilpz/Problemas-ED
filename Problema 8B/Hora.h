...
class Hora{
    ...
    public:
        ...
        Hora(int h=0,int m=0,int s=0):hora(h),minuto(m),segundo(s){
            if(h<0||h>HORAS_MAX) throw std::invalid_argument("ERROR1");
            if(m<0||m>MINUTOS_MAX) throw std::invalid_argument("ERROR2");
            if(s<0||s>SEGUNDOS_MAX) throw std::invalid_argument("ERROR3");
        }
        ...
        Hora operator+(Hora const& h2){
            ...
            if(h>HORAS_MAX) throw std::out_of_range("ERROR4");
            return Hora(h,m,s);
        }
